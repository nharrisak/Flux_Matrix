...
{ .name = "Low Cut", .min = 0, .max = 1000, .def = 0, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "Wetness", .min = 0, .max = 1000, .def = 500, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
};
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	float regen = 0.0002825f;
	int adjPredelay = downRate;
	
	dram->fixA[fix_freq] = 3059.0f/downRate;
	dram->fixA[fix_reso] = 0.0166464f;
	dram->fixC[fix_freq] = dram->fixD[fix_freq] = 217.0f/downRate;
	dram->fixC[fix_reso] = dram->fixD[fix_reso] = 0.0039204f;
	...
	dram->fixC[fix_a1] = dram->fixD[fix_a1] = 0.0f;
	dram->fixC[fix_a2] = dram->fixD[fix_a2] = -dram->fixD[fix_a0];
	dram->fixC[fix_b1] = dram->fixD[fix_b1] = 2.0f * (K * K - 1.0f) * norm;
	dram->fixC[fix_b2] = dram->fixD[fix_b2] = (1.0f - K / dram->fixD[fix_reso] + K * K) * norm;
	...
	while (nSampleFrames-- > 0) {
		...
		if (cycle == cycleEnd) { //hit the end point and we do a reverb sample
			...
			countZ = floor(delay);
			delay -= speed;
			if (delay < 0) delay += (float)adjPredelay;
			float newSampleL = inputSampleL;
			float newSampleR = inputSampleR;
			float incrementL = (inputSampleL - prevSampleL) / speed;
			float incrementR = (inputSampleR - prevSampleR) / speed;
			
			dram->aZL[countZ] = prevSampleL;
			dram->aZR[countZ] = prevSampleR;
			...
			countZ = floor(delay);
			steedRegenL = inputSampleL = dram->aZL[countZ];
			steedRegenR = inputSampleR = dram->aZR[countZ];
