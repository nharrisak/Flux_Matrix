...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	double speed = (pow(1.0-GetParameter( kParam_Two ),3)*80.0)+1.0;
	double steedRegen = pow(GetParameter( kParam_Three ),2)*0.618;
	double iirAmount = ((GetParameter( kParam_Four )+0.02)*1000.0)/downRate;
