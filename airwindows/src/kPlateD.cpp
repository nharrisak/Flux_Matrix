...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	float regen = (GetParameter(kParam_Two)*0.0425f)+0.16f;
	...
	dram->fixA[fix_freq] = 20.0f/downRate;
	dram->fixA[fix_reso] = 0.0018769f;
	dram->fixD[fix_freq] = 14.0f/downRate;
	dram->fixD[fix_reso] = 0.0024964f;
	dram->fixB[fix_freq] = (dram->fixA[fix_freq] + dram->fixA[fix_freq] + dram->fixD[fix_freq]) / 3.0f;
	dram->fixB[fix_reso] = 0.0020834f;	
	dram->fixC[fix_freq] = (dram->fixA[fix_freq] + dram->fixD[fix_freq] + dram->fixD[fix_freq]) / 3.0f;
	dram->fixC[fix_reso] = 0.0022899f;
