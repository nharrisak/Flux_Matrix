...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	dram->highpass[hilp_freq] = ((pow(GetParameter( kParam_HIP ),3)*24000.0f)+10.0f)/GetSampleRate();
	...
	dram->lowpass[hilp_freq] = ((pow(1.0f-GetParameter( kParam_LOP ),3)*24000.0f)+10.0f)/GetSampleRate();
	...
	dram->highpass[hilp_bB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->highpass[hilp_bB2] = (1.0f - K / 1.93185165f + K * K) * norm;
	norm = 1.0f / (1.0f + K / 0.70710678f + K * K);
	dram->highpass[hilp_cB0] = norm;
	dram->highpass[hilp_cB1] = -2.0f * dram->highpass[hilp_cB0];
	dram->highpass[hilp_dB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->highpass[hilp_dB2] = (1.0f - K / 0.70710678f + K * K) * norm;
	norm = 1.0f / (1.0f + K / 0.51763809f + K * K);
	dram->highpass[hilp_eB0] = norm;
	dram->highpass[hilp_eB1] = -2.0f * dram->highpass[hilp_eB0];
	dram->highpass[hilp_fB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->highpass[hilp_fB2] = (1.0f - K / 0.51763809f + K * K) * norm;
	...
	dram->lowpass[hilp_bB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->lowpass[hilp_bB2] = (1.0f - K / 1.93185165f + K * K) * norm;
	norm = 1.0f / (1.0f + K / 0.70710678f + K * K);
	dram->lowpass[hilp_cB0] = K * K * norm;
	dram->lowpass[hilp_cB1] = 2.0f * dram->lowpass[hilp_cB0];
	dram->lowpass[hilp_dB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->lowpass[hilp_dB2] = (1.0f - K / 0.70710678f + K * K) * norm;
	norm = 1.0f / (1.0f + K / 0.51763809f + K * K);
	dram->lowpass[hilp_eB0] = K * K * norm;
	dram->lowpass[hilp_eB1] = 2.0f * dram->lowpass[hilp_eB0];
	dram->lowpass[hilp_fB1] = 2.0f * (K * K - 1.0f) * norm;
	dram->lowpass[hilp_fB2] = (1.0f - K / 0.51763809f + K * K) * norm;
	...
	while (nSampleFrames-- > 0) {
		...
		inputSampleL *= 1.6180339887498949f;
		...
		inputSampleR *= 1.6180339887498949f;
		//ConsoleXBuss after initial Highpass/Lowpass stages
