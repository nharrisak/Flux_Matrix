...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	dram->highpass[hilp_freq] = ((pow(GetParameter( kParam_HIP ),3)*24000.0)+10.0)/GetSampleRate();
	if (dram->highpass[hilp_freq] > 0.495) dram->highpass[hilp_freq] = 0.495;
	bool highpassEngage = true; if (GetParameter( kParam_HIP ) == 0.0) highpassEngage = false;
	
	dram->lowpass[hilp_freq] = ((pow(1.0-GetParameter( kParam_LOP ),3)*24000.0)+10.0)/GetSampleRate();
	if (dram->lowpass[hilp_freq] > 0.495) dram->lowpass[hilp_freq] = 0.495;
	...
	dram->highpass[hilp_aB1] = -2.0 * dram->highpass[hilp_aB0];
	dram->highpass[hilp_bB1] = 2.0 * (K * K - 1.0) * norm;
	dram->highpass[hilp_bB2] = (1.0 - K / 1.93185165 + K * K) * norm;
	norm = 1.0 / (1.0 + K / 0.70710678 + K * K);
	dram->highpass[hilp_cB0] = norm;
	dram->highpass[hilp_cB1] = -2.0 * dram->highpass[hilp_cB0];
	dram->highpass[hilp_dB1] = 2.0 * (K * K - 1.0) * norm;
	dram->highpass[hilp_dB2] = (1.0 - K / 0.70710678 + K * K) * norm;
	norm = 1.0 / (1.0 + K / 0.51763809 + K * K);
	dram->highpass[hilp_eB0] = norm;
	dram->highpass[hilp_eB1] = -2.0 * dram->highpass[hilp_eB0];
	dram->highpass[hilp_fB1] = 2.0 * (K * K - 1.0) * norm;
	dram->highpass[hilp_fB2] = (1.0 - K / 0.51763809 + K * K) * norm;
	
	K = tan(M_PI * dram->lowpass[hilp_freq]); //lowpass
	norm = 1.0 / (1.0 + K / 1.93185165 + K * K);
	dram->lowpass[hilp_aB0] = K * K * norm;
	dram->lowpass[hilp_aB1] = 2.0 * dram->lowpass[hilp_aB0];
	dram->lowpass[hilp_bB1] = 2.0 * (K * K - 1.0) * norm;
	dram->lowpass[hilp_bB2] = (1.0 - K / 1.93185165 + K * K) * norm;
	norm = 1.0 / (1.0 + K / 0.70710678 + K * K);
	dram->lowpass[hilp_cB0] = K * K * norm;
	dram->lowpass[hilp_cB1] = 2.0 * dram->lowpass[hilp_cB0];
	dram->lowpass[hilp_dB1] = 2.0 * (K * K - 1.0) * norm;
	dram->lowpass[hilp_dB2] = (1.0 - K / 0.70710678 + K * K) * norm;
	norm = 1.0 / (1.0 + K / 0.51763809 + K * K);
	dram->lowpass[hilp_eB0] = K * K * norm;
	dram->lowpass[hilp_eB1] = 2.0 * dram->lowpass[hilp_eB0];
	dram->lowpass[hilp_fB1] = 2.0 * (K * K - 1.0) * norm;
	dram->lowpass[hilp_fB2] = (1.0 - K / 0.51763809 + K * K) * norm;
	
	if (dram->highpass[hilp_aA0] == 0.0) { // if we have just started, start directly with raw info
		dram->highpass[hilp_aA0] = dram->highpass[hilp_aB0];
		dram->highpass[hilp_aA1] = dram->highpass[hilp_aB1];
		dram->highpass[hilp_bA1] = dram->highpass[hilp_bB1];
		dram->highpass[hilp_bA2] = dram->highpass[hilp_bB2];
	...
	while (nSampleFrames-- > 0) {
		...
		dram->highpass[hilp_a0] = (dram->highpass[hilp_aA0]*temp)+(dram->highpass[hilp_aB0]*(1.0-temp));
		dram->highpass[hilp_a1] = (dram->highpass[hilp_aA1]*temp)+(dram->highpass[hilp_aB1]*(1.0-temp));
		dram->highpass[hilp_b1] = (dram->highpass[hilp_bA1]*temp)+(dram->highpass[hilp_bB1]*(1.0-temp));
		dram->highpass[hilp_b2] = (dram->highpass[hilp_bA2]*temp)+(dram->highpass[hilp_bB2]*(1.0-temp));
		dram->highpass[hilp_c0] = (dram->highpass[hilp_cA0]*temp)+(dram->highpass[hilp_cB0]*(1.0-temp));
		dram->highpass[hilp_c1] = (dram->highpass[hilp_cA1]*temp)+(dram->highpass[hilp_cB1]*(1.0-temp));
		dram->highpass[hilp_d1] = (dram->highpass[hilp_dA1]*temp)+(dram->highpass[hilp_dB1]*(1.0-temp));
		dram->highpass[hilp_d2] = (dram->highpass[hilp_dA2]*temp)+(dram->highpass[hilp_dB2]*(1.0-temp));
		dram->highpass[hilp_e0] = (dram->highpass[hilp_eA0]*temp)+(dram->highpass[hilp_eB0]*(1.0-temp));
		dram->highpass[hilp_e1] = (dram->highpass[hilp_eA1]*temp)+(dram->highpass[hilp_eB1]*(1.0-temp));
		dram->highpass[hilp_f1] = (dram->highpass[hilp_fA1]*temp)+(dram->highpass[hilp_fB1]*(1.0-temp));
		dram->highpass[hilp_f2] = (dram->highpass[hilp_fA2]*temp)+(dram->highpass[hilp_fB2]*(1.0-temp));
		dram->lowpass[hilp_a0] = (dram->lowpass[hilp_aA0]*temp)+(dram->lowpass[hilp_aB0]*(1.0-temp));
		dram->lowpass[hilp_a1] = (dram->lowpass[hilp_aA1]*temp)+(dram->lowpass[hilp_aB1]*(1.0-temp));
		dram->lowpass[hilp_b1] = (dram->lowpass[hilp_bA1]*temp)+(dram->lowpass[hilp_bB1]*(1.0-temp));
		dram->lowpass[hilp_b2] = (dram->lowpass[hilp_bA2]*temp)+(dram->lowpass[hilp_bB2]*(1.0-temp));
		dram->lowpass[hilp_c0] = (dram->lowpass[hilp_cA0]*temp)+(dram->lowpass[hilp_cB0]*(1.0-temp));
		dram->lowpass[hilp_c1] = (dram->lowpass[hilp_cA1]*temp)+(dram->lowpass[hilp_cB1]*(1.0-temp));
		dram->lowpass[hilp_d1] = (dram->lowpass[hilp_dA1]*temp)+(dram->lowpass[hilp_dB1]*(1.0-temp));
		dram->lowpass[hilp_d2] = (dram->lowpass[hilp_dA2]*temp)+(dram->lowpass[hilp_dB2]*(1.0-temp));
		dram->lowpass[hilp_e0] = (dram->lowpass[hilp_eA0]*temp)+(dram->lowpass[hilp_eB0]*(1.0-temp));
		dram->lowpass[hilp_e1] = (dram->lowpass[hilp_eA1]*temp)+(dram->lowpass[hilp_eB1]*(1.0-temp));
		dram->lowpass[hilp_f1] = (dram->lowpass[hilp_fA1]*temp)+(dram->lowpass[hilp_fB1]*(1.0-temp));
		dram->lowpass[hilp_f2] = (dram->lowpass[hilp_fA2]*temp)+(dram->lowpass[hilp_fB2]*(1.0-temp));
