...
#define AIRWINDOWS_NAME "PointyGuitar"
#define AIRWINDOWS_DESCRIPTION "A supremely adaptable instrument amp."
#define AIRWINDOWS_GUID NT_MULTICHAR( 'A','P','o','j' )
...
{ .name = "Drive", .min = 0, .max = 1000, .def = 600, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "Presnce", .min = 0, .max = 1000, .def = 500, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
...
{ .name = "HSpeakr", .min = 0, .max = 1000, .def = 700, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
...
{ .name = "Output", .min = 0, .max = 1000, .def = 700, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
};
...
#define inNumChannels (1)
{
	...
	while (nSampleFrames-- > 0) {
		...
		for (int x = 0; x < poles; x++) {
			...
			inputSample *= drive;
			inputSample -= fmin(fmax((inputSample*(fabs(inputSample)*0.654f)*(fabs(inputSample)*0.654f)),-1.0f),1.0f);
		}
