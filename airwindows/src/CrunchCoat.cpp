...
#include "../include/template1.h"
	...
	float lastStepR;
	float lastVerb;
	float cycle; //the new undersampling
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	while (nSampleFrames-- > 0) {
		...
		if (cycle > 1.0f) { //hit the end point and we do a reverb sample
			cycle -= 1.0f;
			...
			inputSampleR = (outAR + outER + outIR + outMR)/16.0f;
		...
		if (inputSampleR < -1.0f) inputSampleR = -1.0f;
		lastVerb = (inputSampleL+inputSampleR)*(derez*0.029f);
