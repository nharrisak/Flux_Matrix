...
#define inNumChannels (1)
{
	...
	dram->biquad[biq_a0] = K / dram->biquad[biq_reso] * norm;
	//dram->biquad[biq_a1] = 0.0f; //bandpass can simplify the biquad kernel: leave out this multiply
	dram->biquad[biq_a2] = -dram->biquad[biq_a0];
	dram->biquad[biq_b1] = 2.0f * (K * K - 1.0f) * norm;
