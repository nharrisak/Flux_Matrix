...
{ .name = "Big", .min = 0, .max = 1000, .def = 500, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
{ .name = "Wet", .min = 0, .max = 1000, .def = 500, .unit = kNT_unitNone, .scaling = kNT_scaling1000, .enumStrings = NULL },
};
...
#define inNumChannels (1)
{
	...
	Float32 big = GetParameter( kParam_One );
	Float32 wet = GetParameter( kParam_Two );	
	Float32 vibSpeed = 0.1f;
	Float32 vibDepth = 7.0f;
	Float32 size = (powf(big,2)*75.0f)+25.0f;
	Float32 depthFactor = 1.0f-powf((1.0f-(0.82f-(((1.0f-big)*0.7f)+(size*0.002f)))),4);	
	Float32 blend = 0.955f-(size*0.007f);
	Float32 regen = depthFactor * 0.5f;
	...
	biquadC[0] = biquadB[0] = biquadA[0] = (10000.0f-(big*wet*3000.0f))/GetSampleRate();
