...
#define AIRWINDOWS_DESCRIPTION "Mastering2"
...
void _airwindowsAlgorithm::render( const Float32* inputL, const Float32* inputR, Float32* outputL, Float32* outputR, UInt32 inFramesToProcess ) {
	...
	double iirSide = pow(GetParameter( kParam_A ),3) * (0.1 / overallscale);

	double threshSinew = (0.25+((1.0-GetParameter( kParam_B ))*0.333))/overallscale;
	double depthSinew = 1.0-pow(1.0-GetParameter( kParam_B ),2.0);
	
	double trebleZoom = GetParameter( kParam_C )-0.5;
	...
	double midZoom = GetParameter( kParam_D )-0.5;
	double midGain = (midZoom*fabs(midZoom))+1.0;
	double kalMid = 0.35-(GetParameter( kParam_D )*0.25); //crossover frequency between mid/bass
	double kalSub = 0.45+(GetParameter( kParam_D )*0.25); //crossover frequency between bass/sub
	
	double bassZoom = (GetParameter( kParam_E )*0.5)-0.25;
	double bassGain = (-bassZoom*fabs(bassZoom))+1.0; //control inverted
	double subGain = ((GetParameter( kParam_E )*0.25)-0.125)+1.0;
	if (subGain < 1.0) subGain = 1.0; //very small sub shift, only pos.
	
	double driveIn = (GetParameter( kParam_F )-0.5)+1.0;
	double driveOut = (-(GetParameter( kParam_F )-0.5)*fabs(GetParameter( kParam_F )-0.5))+1.0;
